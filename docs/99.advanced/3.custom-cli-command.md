...
All custom commands must be explicitly registered in `vercube.config.ts` under `cli.commands`. The CLI loads this file using [jiti](https://github.com/unjs/jiti) at startup - TypeScript is transpiled on-the-fly, so no separate build step is needed for your command files.
