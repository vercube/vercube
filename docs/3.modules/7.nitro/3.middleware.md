...
If you want a middleware to run on every request, register it in `GlobalMiddlewareRegistry` via a [`setupFile`](./0.overview#customizing-the-container). See [Global Middlewares](/2.core-features/3.middlewares#global-middlewares) in the core docs for the full API.
