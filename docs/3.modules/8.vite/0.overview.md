...
The generated entry lives at `node_modules/.vercube/server-entry.<hash>.mjs`, so it stays out of source control. The hash comes from the project root, so two dev servers that reach one checkout through different paths (your machine and a Docker container with the project mounted at `/var/www`, for example) each keep their own entry instead of overwriting each other's.
