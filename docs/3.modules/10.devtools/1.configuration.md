...
| `captureLogs`    | `boolean`        | `true`         | Capture events emitted through the `Logger` service and evlog.        |
...
::warning
Bodies are not redacted, because there is no name to match on. Headers and query parameters are, by name. Note that both are attached to the request span, so any OpenTelemetry exporter you have configured receives them too; devtools therefore only enables capture outside production. If your endpoints carry secrets in the payload, either set a `token` so nobody else can read the inspector, or turn `captureBodies` off.
::
