...
Framework plugins extend config, runtime, CLI, and the dev parent process. **Recommended:** classes extending **`BasePlugin`**. **`defineVercubePlugin`** is optional **syntax sugar** for small inline hooks in this file (same pipeline under the hood).

```ts [vercube.config.ts]
import { defineConfig, defineVercubePlugin } from '@vercube/core';
import { FeaturePlugin } from './src/plugins/FeaturePlugin';
...
See [Plugins](/docs/advanced/custom-plugin) for the canonical vs syntax-sugar distinction, **`BasePlugin`** methods, hook order, and where each hook runs (CLI vs worker vs `vercube dev`).
