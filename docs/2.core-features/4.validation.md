...
The same Zod schemas you use for validation can drive **OpenAPI** specs and interactive docs. Install [`@vercube/schema`](/docs/modules/schema/overview), add `SchemaPlugin`, annotate routes with `@Schema`, and open `/_schema/` (JSON) or `/_schema/docs` ([Scalar](https://github.com/scalar/scalar) UI). See the [Schema module](/docs/modules/schema/overview) for setup and the [Scalar](/docs/modules/schema/scalar) page for UI options.